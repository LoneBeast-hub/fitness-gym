    ...
    return(
        <div className="w-[100vw] h-[100vh] flex items-center justify-center bg-[rgba(0,0,0,0.6)] fixed z-10 top-0">
            <div className="w-[90%] items-center relative flex flex-col md:w-[55%] md:max-w-[787px] box-border text-center pt-[2.5rem] md:pt-[5rem] pb-[3.5rem] md:pb-[4.1rem] px-[7%] rounded-[5px] bg-white">
                {/* title */}
                <p className="text-[1.8rem] md:text-[3.2rem] text-red-100 font-bold mb-[1.8rem]">Disable Member</p>
                {/* text */}
                <p className="text-[1.6rem] md:text-[2.8rem] text-black-100 font-medium mb-[2.2rem]">Are you sure you want to disable this member?</p>
                {/* buttons */}
                <div className="flex w-full gap-[1.7rem] md:self-end">
                    ...
                    <CustomButton clickFunction={async () => {
                        ...
                        try {
                            ...
                            if (userId === '8219-3420-34564-7483') {
                                ...
                                if (userId === userProfileResponse.userprofile.userid) {
                                    ...
                                    const disableMemberResult = await fetch(`https://goodnessgfc.com.ng/gymserver/admin/managemembers/disablemember.php`, {
                                        method: 'POST',
                                        body: JSON.stringify({
                                            'userid': userId,
                                            'memberid': contextState.memberIdToDisable,
                                        }),
                                        headers: {
                                            "Content-Type": "application/json; charset=utf-8",
                                            "Accesstoken": accessToken
                                    ...
                                    if (!disableMemberResult.ok) {
                                        // handle non-2xx HTTP responses
                                        throw new Error('Failed to fetch Members Data');
                                    }
                
                                    const disableMemberResponse = await disableMemberResult.json();
                                    console.log(disableMemberResponse)
                                } else {
                                    console.log('Error: Cannot Disable Member data, Reason: User cannot be validated!');
                                }
                            } else {
                                console.log('Error: Cannot Disable Member data, Reason: User cannot be validated!');
                            }
                        ...
                        setContextState((prevValues) => {
                            return {
                                ...prevValues,
                                successMessage: 'Member Successfully Disabled!',
                                showSuccessModal: true,
                                showDisableMemberModal: false,
                            };
                        });
                    }} setType='submit' AddClassName='w-[50%] px-[1rem] rounded-[0.5rem] py-[1.5rem] md:py-[2.5rem]' redColored><span className="text-[1.6rem] md:text-[2rem]">Send</span></CustomButton>
                </div>
                {/* cancel */}
                <div onClick={() => {
                    setContextState((prevValues) => ({
